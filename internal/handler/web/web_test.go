...
// encryptJWE encrypts plaintext with the given RSA public key using
// RSA-OAEP + AES-256-GCM — the same combination used by the Venafi Control
// Plane — and returns a compact-serialised JWE string.
//
// NOTE: this helper uses github.com/go-jose/go-jose/v4, matching the
// production code.  The old gopkg.in/square/go-jose.v2 dependency has been
// fully removed as part of the CWE-409 / CVE-2024-28180 fix.
...
// decryption middleware loaded from keyPath, plus a POST /v1/test handler that
// echoes the decrypted request body back to the caller.
...
// ---------------------------------------------------------------------------
// CWE-409 — decompression-bomb / algorithm allow-list enforcement
// ---------------------------------------------------------------------------
...
//
// go-jose v4's ParseEncrypted returns an error on structurally invalid JWE
// tokens without performing any decompression, closing CWE-409.
func TestMiddleware_RejectsPlaintext(t *testing.T) {
	...
	assert.Equal(t, http.StatusBadRequest, rec.Code,
		"plain-text body must be rejected with 400")
}
...
func TestMiddleware_RejectsWrongKey(t *testing.T) {
	...
	assert.Equal(t, http.StatusUnauthorized, rec.Code,
		"JWE encrypted with the wrong key must be rejected with 401")
}

// TestMiddleware_AcceptsValidJWE verifies the happy path: a request correctly
// encrypted with the server's public key (RSA-OAEP + AES-256-GCM) is decrypted
// and forwarded to the handler intact.
...
// TestMiddleware_RejectsDisallowedKeyAlgorithm is the direct regression test
// for CVE-2024-28180 / CWE-409.
//
// It constructs a JWE token whose "alg" header is RSA1_5 — an algorithm that is
// not present in allowedKeyAlgorithms — and verifies that ParseEncrypted rejects
// it immediately.  Because go-jose v4 validates the algorithm header *before*
// decompressing the ciphertext, a crafted decompression-bomb payload is never
// inflated, preventing the pre-auth DoS.
func TestMiddleware_RejectsDisallowedKeyAlgorithm(t *testing.T) {
	keyPath, pk := writeRSAKeyPEMFile(t)

	// Deliberately encrypt with RSA1_5, which is excluded from allowedKeyAlgorithms.
	enc, err := jose.NewEncrypter(
		jose.A256GCM,
		jose.Recipient{Algorithm: jose.RSA1_5, Key: &pk.PublicKey},
		nil,
	)
	require.NoError(t, err)
	obj, err := enc.Encrypt([]byte("decompression bomb payload"))
	require.NoError(t, err)
	...
	assert.Equal(t, http.StatusBadRequest, rec.Code,
		"JWE with disallowed key algorithm (RSA1_5) must be rejected (CVE-2024-28180 / CWE-409)")
}
